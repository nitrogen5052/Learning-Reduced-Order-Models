...
Version 3.0 adds cross-section assembly from an externally supplied set of
channel bases and reduced coordinates, so a study that spans several physical
systems can reuse one basis instead of one trained emulator per system. Every
2.0 entry point is preserved unchanged.
"""
...
# ==========================================================================
# external-basis cross sections
# ==========================================================================

@dataclass(frozen=True)
class CrossSectionAssembler:
    """Reusable cross-section assembler bound to one physical system."""
...
def cross_section_assembler(
    *,
    emulator,
    bases: Mapping[Any, BasisState],
    angles_degrees: Sequence[float],
) -> CrossSectionAssembler:
    """Bind externally supplied channel bases to one sampled emulator."""
    rose = _import_rose()
    samples = emulator.samples
    if samples is None:
        raise LROMStateError("cross_section_assembler requires a sampled emulator")
    partial_waves = tuple(emulator.partial_waves)
    if partial_waves != tuple(range(max(partial_waves) + 1)):
        raise ValueError(
            "cross-section assembly requires contiguous partial waves starting at l=0"
        )
    angle_array = np.asarray(angles_degrees, dtype=float)
    if angle_array.ndim != 1 or angle_array.size == 0:
        raise ValueError("angles_degrees must be a non-empty one-dimensional array")
    ...
    for ell in partial_waves:
        ...
        for spin_index in range(len(interaction_row)):
            key = _interaction_channel_key(interaction_row, ell, spin_index)
            if key not in bases:
                raise ValueError(f"no basis supplied for channel {key}")
            basis_state = bases[key]
            if np.asarray(basis_state.phi0).size != rho_mesh.size:
                raise ValueError(
                    "supplied bases and the emulator mesh must have equal length"
                )
            model = samples.full_order_models[key]
            custom_basis = rose.basis.CustomBasis(
                solutions=np.column_stack((basis_state.phi0, basis_state.vectors)),
                phi_0=np.asarray(basis_state.phi0, dtype=np.complex128),
                rho_mesh=rho_mesh,
                n_basis=basis_state.basis_size,
                solver=model.solver,
                subtract_phi0=True,
...
def smatrix_from_coefficients(
    *,
    assembler: CrossSectionAssembler,
    coefficients: Mapping[Any, np.ndarray],
) -> SmatrixState:
    """Convert supplied channel coordinates into a packed S matrix."""
    rbes = []
    ell_slots = []
    spin_slots = []
    for ell in assembler.partial_waves:
        for spin_index, rbe in enumerate(assembler.sae.rbes[ell]):
            rbes.append(rbe)
            ell_slots.append(ell)
            spin_slots.append(spin_index)
    if len(rbes) != len(assembler.channel_keys):
        raise ValueError("assembler channels do not match the bound emulator")

    coordinate_array = np.stack(
        [np.asarray(coefficients[key], dtype=np.complex128) for key in assembler.channel_keys],
        axis=1,
    )
    ones = np.ones((*coordinate_array.shape[:2], 1), dtype=np.complex128)
    expansion = np.concatenate([ones, coordinate_array], axis=2)
    asymptotic_values = np.asarray([rbe.asymptotic_vals for rbe in rbes], dtype=np.complex128)
    asymptotic_derivatives = np.asarray([rbe.asymptotic_ders for rbe in rbes], dtype=np.complex128)
    phi = np.einsum("scb,cb->sc", expansion, asymptotic_values, optimize=True)
    phi_prime = np.einsum("scb,cb->sc", expansion, asymptotic_derivatives, optimize=True)
    s_0 = np.asarray([rbe.s_0 for rbe in rbes])
    r_matrix = phi / (s_0[np.newaxis, :] * phi_prime)
    s_flat = (
        np.asarray([rbe.Hm for rbe in rbes])[np.newaxis, :]
        - s_0[np.newaxis, :] * r_matrix * np.asarray([rbe.Hmp for rbe in rbes])[np.newaxis, :]
    ) / (
        np.asarray([rbe.Hp for rbe in rbes])[np.newaxis, :]
        - s_0[np.newaxis, :] * r_matrix * np.asarray([rbe.Hpp for rbe in rbes])[np.newaxis, :]
    )
    splus = np.zeros(
        (coordinate_array.shape[0], len(assembler.partial_waves)), dtype=np.complex128
    )
    sminus = np.zeros_like(splus)
    for offset, (ell, spin_index) in enumerate(zip(ell_slots, spin_slots)):
        if spin_index == 0:
            splus[:, ell] = s_flat[:, offset]
            if ell == 0:
                sminus[:, ell] = s_flat[:, offset]
            ...
            sminus[:, ell] = s_flat[:, offset]
    return SmatrixState(
        partial_waves=assembler.partial_waves, splus=splus, sminus=sminus
    )


def cross_sections_from_coefficients(
    *,
    assembler: CrossSectionAssembler,
    values: np.ndarray,
    coefficients: Mapping[Any, np.ndarray],
) -> CrossSectionState:
    """Assemble differential cross sections from supplied coordinates."""
    smatrix = smatrix_from_coefficients(assembler=assembler, coefficients=coefficients)
    rows = np.asarray(values, dtype=float)
    cross_sections = [
        assembler.sae.calculate_xs(splus, sminus, row).dsdo
        for row, splus, sminus in zip(rows, smatrix.splus, smatrix.sminus)
    ]
    return CrossSectionState(
        angles_degrees=assembler.angles_degrees,
        values=np.asarray(cross_sections, dtype=float),
    )


def exact_smatrix(
    *, assembler: CrossSectionAssembler, values: np.ndarray
) -> SmatrixState:
    """Full-order S matrix from the assembler's per-channel exact solvers."""
    rows = np.asarray(values, dtype=float)
    channel_count = len(assembler.partial_waves)
    splus = np.empty((rows.shape[0], channel_count), dtype=np.complex128)
    sminus = np.empty_like(splus)
    for index, row in enumerate(rows):
        for ell, rbe_row in enumerate(assembler.sae.rbes):
            splus[index, ell] = rbe_row[0].basis.solver.smatrix(row)
            sminus[index, ell] = (
                splus[index, ell] if ell == 0 else rbe_row[1].basis.solver.smatrix(row)
            )
    return SmatrixState(
        partial_waves=assembler.partial_waves, splus=splus, sminus=sminus
    )
...
def exact_cross_sections(
    *, assembler: CrossSectionAssembler, values: np.ndarray
) -> CrossSectionState:
    """Full-order differential cross sections on the assembler's angle grid."""
    rows = np.asarray(values, dtype=float)
    smatrix = exact_smatrix(assembler=assembler, values=rows)
    return CrossSectionState(
        angles_degrees=assembler.angles_degrees,
        values=np.asarray(
            [
